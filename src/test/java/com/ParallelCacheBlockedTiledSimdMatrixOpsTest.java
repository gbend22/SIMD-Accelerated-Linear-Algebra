...
class ParallelCacheBlockedTiledSimdMatrixOpsTest {
    ...
    void multiply_knownValue() {
        ...
        float[][] result = parallelCacheBlocked.multiply(a, b);
    ...
    void multiply_identityReturnsOriginal() {
        ...
        float[][] result = parallelCacheBlocked.multiply(a, id);
    ...
    @ParameterizedTest
    @ValueSource(ints = {1, 2, 7, 16, 33, 64, 65, 100, 129, 200, 260, 300})
    ...
    void multiply_matchesScalar_rectangular() {
        float[][] a = random(260, 200, 7);
        float[][] b = random(200, 230, 11);
        float[][] expected = scalar.multiply(a, b);
        float[][] result = parallelCacheBlocked.multiply(a, b);
        for (int i = 0; i < a.length; i++) {
            assertArrayEquals(expected[i], result[i], DELTA);
    ...
    void multiply_mismatchedDimensions_throws() {
        float[][] a = {{1, 2, 3}, {4, 5, 6}};
        float[][] b = {{1, 2}, {3, 4}};
        assertThrows(IllegalArgumentException.class, () -> parallelCacheBlocked.multiply(a, b));
    }
}
