...
class RegisterTileSweepMatrixOpsTest {

    private static final float DELTA = 1e-3f;
    private static final int[] MR_VALUES = {1, 2, 4, 6, 8};
    ...
    void multiply_knownValue() {
        ...
        for (int mr : MR_VALUES) {
            float[][] result = sweep.multiply(a, b, mr);
            for (int i = 0; i < expected.length; i++) {
                assertArrayEquals(expected[i], result[i], DELTA, "mr=" + mr);
            }
        }
    }

    @ParameterizedTest
    @ValueSource(ints = {1, 2, 5, 7, 16, 33, 64, 65, 100, 129})
    void multiply_matchesScalar_square(int n) {
        ...
        for (int mr : MR_VALUES) {
            float[][] result = sweep.multiply(a, b, mr);
            for (int i = 0; i < n; i++) {
                assertArrayEquals(expected[i], result[i], DELTA, "mr=" + mr + " row=" + i);
            }
        }
    }
    ...
    void multiply_mismatchedDimensions_throws() {
        float[][] a = {{1, 2, 3}, {4, 5, 6}};
        float[][] b = {{1, 2}, {3, 4}};
        assertThrows(IllegalArgumentException.class, () -> sweep.multiply(a, b, 4));
    }

    @Test
    void multiply_unsupportedMr_throws() {
        float[][] a = {{1, 2}, {3, 4}};
        float[][] b = {{5, 6}, {7, 8}};
        assertThrows(IllegalArgumentException.class, () -> sweep.multiply(a, b, 3));
    }
}
