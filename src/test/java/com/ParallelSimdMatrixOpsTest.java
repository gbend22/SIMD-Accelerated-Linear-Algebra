...
class ParallelSimdMatrixOpsTest {
    ...
    void multiply_knownValue() {
        ...
        float[][] result = parallel.multiply(a, b);
    ...
    void multiply_identityReturnsOriginal() {
        ...
        float[][] result = parallel.multiply(a, id);
    ...
    @ParameterizedTest
    @ValueSource(ints = {1, 2, 7, 16, 33, 64, 100})
    ...
    void multiply_matchesScalar_rectangular() {
        float[][] a = random(40, 25, 7);
        float[][] b = random(25, 17, 11);
        float[][] expected = scalar.multiply(a, b);
        float[][] result = parallel.multiply(a, b);
        for (int i = 0; i < a.length; i++) {
            assertArrayEquals(expected[i], result[i], DELTA);
    ...
    void multiply_mismatchedDimensions_throws() {
        float[][] a = {{1, 2, 3}, {4, 5, 6}};
        float[][] b = {{1, 2}, {3, 4}};
        assertThrows(IllegalArgumentException.class, () -> parallel.multiply(a, b));
    }
}
